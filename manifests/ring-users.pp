class ring_users {

##### BEGIN intouch #####

    add_user { intouch:
        email    => "tech@intouch.eu",
        company => "InTouch NV",
        uid      => 5001,
        groups  => ['ring-users'],
    }
    add_ssh_key { intouch:
        user    => "intouch",
        key     => "",
        name    => "tt@intouch.eu",
        type    => "ssh-rsa",
        options => [],
    }
    add_ssh_key { kevin_intouch:
        user    => "intouch",
        key     => "",
        name    => "kevin@intouch.eu",
        type    => "ssh-rsa",
        options => [],
    }


##### END intouch ##### 

##### BEGIN bit #####

    add_user { bit:
        email    => "teun@teun.tv",
        company => "BIT",
        uid      => 5002,
        groups   => ['ring-users']
    }
    add_ssh_key { bit_ssh_1:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "alex@vali_1",
        options => [],
    }      
    add_ssh_key { bit_ssh_2:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "bart@bart-desktop_2",
        options => [],
    }      
    add_ssh_key { bit_ssh_3:
        user => "bit",
        type => "ssh-dss",
        key => "",
        name => "bastiaan@bastiaan-desktop_3",
        options => [],
    }      
    add_ssh_key { bit_ssh_4:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "emil@barabas_4",
        options => [],
    }      
    add_ssh_key { bit_ssh_5:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "henkjan@henkjan-desktop_5",
        options => [],
    }      
    add_ssh_key { bit_ssh_6:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "juul@juul-desktop_6",
        options => [],
    }      
    add_ssh_key { bit_ssh_7:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "martijn@martijn-desktop_7",
        options => [],
    }      
    add_ssh_key { bit_ssh_8:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "sanders@sanders-desktop_8",
        options => [],
    }      
    add_ssh_key { bit_ssh_9:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "stefan@toaster_9",
        options => [],
    }      
    add_ssh_key { bit_ssh_10:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "teun@moridin_10",
        options => [],
    }      
    add_ssh_key { bit_ssh_11:
        user => "bit",
        type => "ssh-rsa",
        key => "",
        name => "jeroen@jeroen-desktop_11",
        options => [],
    }  
##### END bit #####

##### BEGIN coloclue #####

    add_user { coloclue:
        email   => "routers@coloclue.net",
        company => "Netwerkvereniging Coloclue",
        uid     => 5003,
        groups  => ['ring-users']
    }
    add_ssh_key { coloclue_ssh_1:
        user => "coloclue",
        type => "ssh-rsa",
        key => "",
        name => "weerd@weirdnet.nl",
        options => [],
    }      
    add_ssh_key { coloclue_ssh_2:
        user => "coloclue",
        type => "ssh-rsa",
        key => "",
        name => "job@instituut.net",
        options => [],
    }      
    add_ssh_key { coloclue_ssh_3:
        user => "coloclue",
        type => "ssh-rsa",
        key => "",
        name => "niels@alphix.local",
        options => [],
    } 
    add_ssh_key { coloclue_ssh_4:
        user    => "coloclue",
        key     => "",
        type    => "ssh-dss",
        name    => "evharten@macbook-pro-van-evert-van-harten.local_1",
        options => [],
    }

##### END coloclue #####

##### BEGIN widexs #####

    add_user { widexs:
        email    => "noc@widexs.nl",
        company => "WideXS",
        uid      => 5004,
        groups   => ['ring-users']
    }
    add_ssh_key { widexs:
        user    => "widexs",
        key     => "",
        name    => "wouter@widexs",
        type    => "ssh-rsa",
        options => [],
    }

##### END widexs #####

##### BEGIN xlshosting #####

    add_user { xlshosting:
        email    => "noc@xlshosting.nl",
        company => "XLSHostin BV",
        uid      => 5005,
        groups   => ['ring-users']
    }
    add_ssh_key { xlshosting:
        user    => "xlshosting",
        key     => "",
        name    => "root@jumphost.xlshosting.nl",
        type    => "ssh-rsa",
        options => [],
    }

##### END xlshosting #####

##### BEGIN interconnect #####

    add_user { interconnect:
        email    => "noc@interconnect.nl",
        company => "Interconnect",
        uid      => 5006,
        groups   => ['ring-users']
    }
    add_ssh_key { interconnect:
        user    => "interconnect",
        key     => "",
        name    => "interconnect@pjotr.interconnect.nl",
        type    => "ssh-dss",
        options => [],
    }

##### END interconnect #####

##### BEGIN cambrium #####

    add_user { cambrium:
        email    => "noc@cambrium.nl",
        company => "Cambrium",
        uid      => 5007,
        groups   => ['ring-users']
    }

    add_ssh_key { cambrium_ssh_1:
        user => "cambrium",
        type => "ssh-rsa",
        key => "",
        name => "cambrium@cambrium01.ring.nlnog.net-rsa",
        options => [],
    }
    add_ssh_key { cambrium_ssh_2:
        user => "cambrium",
        type => "ssh-dss",
        key => "",
        name => "cambrium@cambrium01.ring.nlnog.net-dss",
        options => [],
    }
    add_ssh_key { cambrium_ssh_3:
        user => "cambrium",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABIwAAAIEAougxtaZAyGf1Hk5gZxUs6VSSaV+cquTW9hrX3Cc1g1sNrNj4Z2PjAnY9L4JDjhcQ9yegg8I/x459gYh22TuEEkzNpxou1N2PSXFPj4vu9eIRyAVaGPbRJLJRCJdjdUNo/xcpprxCYm6i+sqf4zoKTHOWA7eKxETqM2Mg1F17ANc=",
        name => "/home/markvh/.ssh/id_rsa",
        options => [],
    }
    add_ssh_key { cambrium_ssh_4:
        user => "cambrium",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABIwAAAIEAwreIddtAA7ONovBfx2Uxt1ZA18aTomnFuynzmi/NOD+BdCHCO0xfUPyYjEPf5+R8iuNBSrjDscMKMFIO2nHShmNFueuCyCvWOYhLCFEEFA1SlmzGa3grAnNqNH71sru7igc30qtAi6/BTO591kvzB+qatxQcO2AEsxALu8aPwRk=",
        name => "/home/staff/johan/.ssh/id_rsa",
        options => [],
    }
    add_ssh_key { cambrium_ssh_5:
        user => "cambrium",
        type => "ssh-dss",
        key => "",
        name => "/home/staff/johan/.ssh/id_dsa",
        options => [],
    }
    add_ssh_key { cambrium_ssh_6:
        user => "cambrium",
        type => "ssh-rsa",
        key => "",
        name => "larsgn@atari",
        options => [],
    }

##### END cambrium #####

##### BEGIN zylon #####


    add_user { zylon:
        email    => "smite@zylon.net",
        company => "Zylon",
        uid      => 5008,
        groups   => ['ring-users']
    }
    add_ssh_key { zylon:
        user    => "zylon",
        key     => "",
        name    => "smite@zylon.net",
        options => [],
        type    => "ssh-dss"
    }
    
##### END zylon #####
    
##### BEGIN duocast #####    
    
    add_user { duocast:
        email   => "noc@duocast.net",
        company => "Duocast",
        uid     => 5009,
        groups  => ['ring-users']
    }
    add_ssh_key { duocast_ssh_1:
        user => "duocast",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABJQAAAIEAghMY9EavxopexZ0HhJvzDYBdIvcf4iBPzvfLrYshuF4keakyugfQ6mob2qlssVIAztJRrQfUDtgvNGKxOFrCzV9vXwf5bJUbCPbUIoSQTdoSBzwl31Q6dHpYBc5jQ5Xmez8wcc0dtxdMun+/eVOVG+ErghC8jpoCVQrwu5ikc2E=",
        name => "fabian@fabian.office.duocast.nl_1",
        options => [],
    }      
    add_ssh_key { duocast_ssh_2:
        user => "duocast",
        type => "ssh-dss",
        key => "",
        name => "tozz@duocast_2",
        options => [],
    }

##### END duocast #####
    
##### BEGIN easyhosting #####

    add_user { easyhosting:
        email   => "systeembeheer@easyhosting.nl",
        company => "Easyhosting",
        uid     => 5010,
        groups  => ['ring-users']
    }
    add_ssh_key { easyhosting:
        user    => "easyhosting",
        key     => "",
        name    => "systeembeheer@easyhosting.nl",
        options => [],
        type    => "ssh-dss",
    }

##### END easyhosting #####
    
##### BEGIN previder #####

    add_user { previder:
        email   => "noc@previder.net",
        company => "Previder",
        uid     => 5011,
        groups  => ['ring-users']
    }

    add_ssh_key { previder_ssh_1:
        user => "previder",
        type => "ssh-dss",
        key => "",
        name => "robin@previder.net",
        options => [],
    }      
    add_ssh_key { previder_ssh_2:
        user => "previder",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABIwAAAIEAsNdT8O09tlD93dRZS3bHxGeSwMlO9n/+ZaHlOXeLtnGd9IvNVe+4UxEcXNK7hHgkJrthhlhT/TEQN6irawfRANZw82HprEjQXOOWKprPXUKqhoGfkBI0FV+t+AzuLxH2gDsDQ/A7aQteKoAJYE4rT1nO6nDJM+/bAosSYBCFANU=",
        name => "nick@mustang.introweb.nl",
        options => [],
    }      
    add_ssh_key { previder_ssh_3:
        user => "previder",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABIwAAAIEA6inD1YOQCwDt7u4EJiMkSWhaC/qlatyExWaO0gpY+cKH3rsGxzibLDpn06f3luBP4fhKPJt5QTNR8jqjrCdmjVqBX1Ox8rp1g+JYW4r69Prwoi1og6FVHQGjT+1sLwoVGDOh+ZhY9Ef8VHpfh7QCkHxv8SEsEFTtlzxMAOwxCkE=",
        name => "edwin@mustang.introweb.nl",
        options => [],
    }
    add_ssh_key { previder_ssh_4:
        user => "previder",
        type => "ssh-dss",
        key => "",
        name => "erik@mustang.introweb.nl",
        options => [],
    }  
 
##### END previder #####
    
##### BEGIN leaseweb #####

    add_user { leaseweb:
        email   => "noc@leaseweb.com",
        company => "Leaseweb BV",
        uid     => 5012,
        groups  => ['ring-users']
    }

    add_ssh_key { leaseweb_ssh_1:
        user => "leaseweb",
        type => "ssh-dss",
        key => "",
        name => "bas@louise.frogerstraat.nl",
        options => [],
    }      
    add_ssh_key { leaseweb_ssh_2:
        user => "leaseweb",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABJQAAAIEAkCgRUkL3e3vm1UO+mQMYhnxdjOavmZXBeBWQO5X9T0y7uj70M+bX/Z3lhOF4Lq0VX4UL8UEfRy/aYa6zxv+SmUsvrPs4Lgo+yeLAVR+zxnZLjsch6r6zFV6+v4dPfKJUO+58bI69+gIUctuczwQn9nvmp76uPbsTvGXVdbtuwzc=",
        name => "GJA",
        options => [],
    }      
    add_ssh_key { leaseweb_ssh_3:
        user => "leaseweb",
        type => "ssh-rsa",
        key => "",
        name => "RDB",
        options => [],
    }      
    add_ssh_key { leaseweb_ssh_4:
        user => "leaseweb",
        type => "ssh-rsa",
        key => "",
        name => "HIJ",
        options => [],
    }      
    add_ssh_key { leaseweb_ssh_5:
        user => "leaseweb",
        type => "ssh-rsa",
        key => "",
        name => "HBL",
        options => [],
    }      
    add_ssh_key { leaseweb_ssh_6:
        user => "leaseweb",
        type => "ssh-rsa",
        key => "",
        name => "VLB",
        options => [],
    }      
    add_ssh_key { leaseweb_ssh_7:
        user => "leaseweb",
        type => "ssh-rsa",
        key => "",
        name => "TOL",
        options => [],
    }      
    add_ssh_key { leaseweb_ssh_8:
        user => "leaseweb",
        type => "ssh-rsa",
        key => "",
        name => "KDE",
        options => [],
    }    

##### END leaseweb #####
    
##### BEGIN nxs #####

    add_user { nxs:
        email   => "ralph.smit@nxs.nl",
        company => "NXS",
        uid     => 5013,
        groups  => ['ring-users']
    }
    add_ssh_key { nxs:
        user    => "nxs",
        key     => "AAAAB3NzaC1yc2EAAAADAQABAAAAgQC+WX1MzDMc0aKMuafHCsmT0tYSMBC1qb2pwCsCqgnskQDvTAk5cjlMwotptbFAUUOxjLY0vEP0KoEh8OFjkJAw9vMlj/X+H5FcWymoNzWePg/mmUGZH2hBWjNCCGuJnyJGPuee0P2zgmQ2MmVLU35hBRk1nEJ7K1AngOGPerY4Iw==",
        name    => "ralphs@nxs",
        options => [],
        type    => "ssh-rsa",
    }

##### END nxs #####
    
##### BEGIN ic-hosting #####

    add_user { ic-hosting:
        email   => "info@ic-hosting.nl",
        company => "IC Hosting",
        uid     => 5014,
        groups  => ['ring-users']
    }
    add_ssh_key { ic-hosting:
        user    => "ic-hosting",
        key     => "",
        name    => "info@ic-hosting.nl",
        options => [],
        type    => "ssh-dss"
    }

##### END ic-hosting #####

##### BEGIN xs4all ##### 
    
    add_user { xs4all:
        email   => "noc@xs4all.net",
        company => "XS4All",
        uid     => 5015,
        groups  => ['ring-users']
    }

    add_ssh_key { xs4all_ssh_1:
        user => "xs4all",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABIwAAAIEAzq+/8IJputPveyQKOFufRTZREZ0uE/P7u0iYbdmMZFsdy56Wt6/Qvkzny/f9AKcHqFuwkDgvYIWtVeZNKa1aL1iWqyjbC9EQkVq37vEzmnWLJRk968W9owoFV8C1JE/iQd5/ZfoNUzcolxd2jTbuvdQ8LiDfEivfbOroi7KwfMs=",
        name => "maarten@macbook.carels.info_1",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_2:
        user => "xs4all",
        type => "ssh-dss",
        key => "",
        name => "mikevs@xs4all.net_2",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_3:
        user => "xs4all",
        type => "ssh-rsa",
        key => "",
        name => "carelsm@netadmin.xs4all.nl_3",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_4:
        user => "xs4all",
        type => "ssh-dss",
        key => "",
        name => "john@pc.xs4all.nl_4",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_5:
        user => "xs4all",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABIwAAAIEA8dDGdThW8WV/k6c+XIpZGcv9IsK067cgT88alOQtaMo/cxCpX2ESWo++RCAwc9SUWxjrsOvo3x6JWGoVOTdXq4SnBwqgX02zZGoL1DjZwfQxfF3zN/AC+uKZTqgX1RnbFfr571TO9laTnJ3dSu5pgElsBAOfURaVGz19ZhN98Ds=",
        name => "Carelsm@beechnut.xs4all.nl_5",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_6:
        user => "xs4all",
        type => "ssh-dss",
        key => "",
        name => "remcovz@shell1.office.xs4all.nl_6",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_7:
        user => "xs4all",
        type => "ssh-dss",
        key => "",
        name => "kiersb@mescaline_7",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_8:
        user => "xs4all",
        type => "ssh-dss",
        key => "",
        name => "jcqschuu@shell1.office.xs4all.nl_8",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_9:
        user => "xs4all",
        type => "ssh-rsa",
        key => "",
        name => "timr@netadmin.xs4all.net_9",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_10:
        user => "xs4all",
        type => "ssh-rsa",
        key => "",
        name => "arndm@netadmin.xs4all.net_10",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_11:
        user => "xs4all",
        type => "ssh-dss",
        key => "",
        name => "timo@timo-laptop_11",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_12:
        user => "xs4all",
        type => "ssh-rsa",
        key => "",
        name => "arndm@mabuntu_12",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_13:
        user => "xs4all",
        type => "ssh-dss",
        key => "",
        name => "johnpc@lsd_13",
        options => [],
    }      
    add_ssh_key { xs4all_ssh_14:
        user => "xs4all",
        type => "ssh-rsa",
        key => "",
        name => "Daniël_14",
        options => [],
    }

##### END xs4all #####

##### BEGIN nedzone #####

    add_user { nedzone:
        email   => "noc@nedzone.nl",
        company => "Nedzone BV",
        uid     => 5016,
        groups  => ['ring-users'],
    }

    add_ssh_key { nedzone_bram:
        user    => "nedzone",
        key     => "AAAAB3NzaC1yc2EAAAABJQAAAIBr1P6kiPiQT+xCJVAmMQtL8VOerIv7epknE3n2ml2rcdlmWIMizGEtRI70rLZmBqyU7EMEU0c+hiq/k/HuzU9FW2Vla1VWi4pUtiqmwQK8u6uHd7tMKF7Vju46U3AWTItDtETIbPkSQyqx9Im1KZGj8FHrDlJ/lujk1olMjNmocw==",
        name    => "bram-rsa-key-20110201",
        options => [],
        type    => "ssh-rsa",
    }

##### END nedzone #####

##### BEGIN nlnogbot #####

    add_user { nlnogbot:
        email   => "sten@snore.nl",
        company => "Sten Spans limited",
        uid     => "5017",
        groups  => ['ring-users'],
    }

    add_ssh_key { nlnogbot_sten:
        user    => "nlnogbot",
        key     => "",
        name    => "sten-nlnogbot@zem.blinkenlights.nl",
        options => [],
        type    => "ssh-rsa",
    }

##### END nlnogbot #####

##### BEGIN oxilion #####

    add_user { oxilion:
        email   => "noc@oxilion.nl",
        company => "Oxilion",
        uid     => "5018",
        groups  => ['ring-users'],
    }
    
    add_ssh_key { oxilion_ssh_1:
        user => "oxilion",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABJQAAAIBlZkdBFEeyjt41M+JefGGTz+qZNU0IqH3yQ7bQC6GccJ7OqXsrHIkw6v1wqKxxp7fZgROxTlERQiV8CU9EHRytRw1oB1AXugt+gYd48qS3m7/e7ldUfMQ4bCZ7geyAbDuxCScpmBh5xpe52zALSgw45HTEvL+RM21sGuwOrIgiUw==",
        name => "rsa-key-20100201",
        options => [],
    }      
    add_ssh_key { oxilion_ssh_2:
        user => "oxilion",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABJQAAAIBR3Y5KY3uInywLRDtdzFelAzO9tMFZ9MVwEIuFCfOx3KtusIz7kArynuS4y4r6K0k/4M9/WlFBkKrr9qcWeoDkwBngn6NZj2a5dw0ImwDUH8CrpoSLFaOh7B46Y4A2aDwEL6Blh5+YgHERcoOVF9ozHYwhxD1SF8FnY8wZST2yzw==",
        name => "arnoud@oxilion",
        options => [],
    }      
    add_ssh_key { oxilion_ssh_3:
        user => "oxilion",
        type => "ssh-rsa",
        key => "",
        name => "ekohl",
        options => [],
    }      
    add_ssh_key { oxilion_ssh_4:
        user => "oxilion",
        type => "ssh-rsa",
        key => "",
        name => "tjikkun@sleptop.hoentjen.local",
        options => [],
    }      
    add_ssh_key { oxilion_ssh_6:
        user => "oxilion",
        type => "ssh-rsa",
        key => "",
        name => "tjikkun@peecee.hoentjen.eu",
        options => [],
    }      
    add_ssh_key { oxilion_ssh_7:
        user => "oxilion",
        type => "ssh-dss",
        key => "",
        name => "sander@fedora.jronline.nl",
        options => [],
    }      
    add_ssh_key { oxilion_ssh_8:
        user => "oxilion",
        type => "ssh-dss",
        key => "",
        name => "lj-key-20080618",
        options => [],
    }      
    add_ssh_key { oxilion_ssh_9:
        user => "oxilion",
        type => "ssh-dss",
        key => "",
        name => "tekkel@fedora.jronline.nl",
        options => [],
    }      
##### END oxilion #####

##### BEGIN ebayclassifiedsgroup #####

    add_user { ebayclassifiedsgroup:
        email   => "DL-eBay-Classifieds-Networking@ebay.com",
        company => "eBay Classifieds Group",
        uid     => "5019",
        groups  => ['ring-users'],
    }
    
    add_ssh_key { ebayclassifiedsgroup_maarten:
        user    => "ebayclassifiedsgroup",
        key     => "",
        name    => "mmoerman@l-lwr-00122309.corp.ebay.com",
        options => [],
        type    => "ssh-rsa",
    }

    add_ssh_key { ebayclassifiedsgroup_koen:
        user    => "ebayclassifiedsgroup",
        key     => "",
        name    => "kvdhulst-ebayclassifiedsgroup",
        options => [],
        type    => "ssh-rsa",
    }

    add_ssh_key { ebayclassifiedsgroup_willem:
        user    => "ebayclassifiedsgroup",
        key     => "",
        name    => "willem_ebay",
        options => [],
        type    => "ssh-dss",
    }

##### END ebayclassifiedsgroup #####

##### BEGIN evertjenetworks #####

    add_user { evertjenetworks:
        email   => "evert@vanharten.nu",
        company => "Evertje Networks",
        uid     => "5020",
        groups  => ['ring-users'],
        ensure  => absent,
    }

    add_ssh_key { evertjenetworks_evert:
        user    => "evertjenetworks",
        key     => "",
        type    => "ssh-dss",
        name    => "evharten@macbook-pro-van-evert-van-harten.local",
        options => [],
        ensure => absent,
    }

##### END evertjenetworks #####

##### BEGIN is #####

    add_user { is:
        email   => "netwerk@is.nl",
        company => "IS",
        uid     => "5021",
        groups  => ['ring-users'],
    }

    add_ssh_key { is_ssh_1:
        user => "is",
        type => "ssh-rsa",
        key => "",
        name => "admpins04@shell01.pinshosting.dir",
        options => [],
    }      
    add_ssh_key { is_ssh_2:
        user => "is",
        type => "ssh-dss",
        key => "",
        name => "matthijs@is.nl",
        options => [],
    }      
    add_ssh_key { is_ssh_3:
        user => "is",
        type => "ssh-dss",
        key => "",
        name => "matthijs@ls-la.nl",
        options => [],
    } 

##### END is #####

##### BEGIN surfnet #####

    add_user { surfnet:
        email   => "surf-ring@prolocation.eu",
        company => "SURFNet",
        uid     => "5022",
        groups  => ['ring-users'],
    }

    add_ssh_key { surfnet_prolokey:
        user    => "surfnet",
        key     => "",
        type    => "ssh-rsa",
        name    => "prolo@surfnet01.ring.nlnog.net",
        options => [],
    }

##### END surfnet #####

##### BEGIN prolocation #####

    add_user { prolocation:
        email   => "cedric@prolocation.net",
        company => "Prolocation",
        uid     => "5023",
        groups  => ['ring-users'],
    }

    add_ssh_key { prolocation_sshkey:
        user    => "prolocation",
        type    => "ssh-rsa",
        key     => "",
        name    => "prolo@prolocation01.ring.nlnog.net",
        options => [],
    }

##### END prolocation #####

##### BEGIN in2ip #####

    add_user { in2ip:
        email   => "noc@in2ip.nl",
        company => "IN2IP",
        uid     => "5024",
        groups  => ['ring-users'],
    }

    add_ssh_key { in2ip_dirkjan:
        user    => "in2ip",
        type    => "ssh-rsa",
        key     => "",
        name    => "dirkjan@nmc",
        options => [],
    }

##### END in2ip #####

##### BEGIN netground #####

    add_user { netground:
        email   => "systeembeheer@netground.nl",
        company => "NetGround BV",
        uid     => "5025",
        groups  => ['ring-users'],
    }

    add_ssh_key { netground_ssh_1:
        user => "netground",
        type => "ssh-dss",
        key => "",
        name => "Net_Ground",
        options => [],
    }

##### END netground #####

##### BEGIN totaalnet #####

    add_user { totaalnet:
        email   => "noc@gl-ix.net",
        company => "Totaalnet",
        uid     => "5026",
        groups  => ['ring-users'],
    }
    add_ssh_key { totaalnet_ssh_1:
        user => "totaalnet",
        type => "ssh-dss",
        key => "",
        name => "dsa-key-20080204_1",
        options => [],
    }      
    add_ssh_key { totaalnet_ssh_2:
        user => "totaalnet",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABJQAAAIB7OjD4FexzjUO+/FQ2Ao7vdbxS6ARqCdtwm1aRZKgCAOIYQatTEeWeCecOYyJ312uEFAFW6M5yTpDnW+8B5SmOSelLSs1CdEgNLHKj5Wwo7qZx8hnIleFGcxnqpMHQiwWkmaJw2FJ/4g1pDDWR+vv5F9wNxpn0bLwdCUC7zKo3mw==",
        name => "rsa-key-20080204_2",
        options => [],
    }      
    add_ssh_key { totaalnet_ssh_3:
        user => "totaalnet",
        type => "ssh-rsa",
        key => "",
        name => "jap@jap-laptop_3",
        options => [],
    }      
    add_ssh_key { totaalnet_ssh_4:
        user => "totaalnet",
        type => "ssh-rsa",
        key => "",
        name => "freek@Blade_4",
        options => [],
    }  

##### END totaalnet #####

##### BEGIN signet #####

    add_user { signet:
       email    => "noc@signet.nl",
       company => "SIGNet",
       uid      => "5027",
       groups   => ['ring-users'],
    }
    add_ssh_key { signet_ssh_1:
        user => "signet",
        type => "ssh-rsa",
        key => "",
        name => "justin@justin-desktop_1",
        options => [],
    }      
    add_ssh_key { signet_ssh_2:
        user => "signet",
        type => "ssh-rsa",
        key => "",
        name => "post@keesmeijs.nl_2",
        options => [],
    }      
    add_ssh_key { signet_ssh_3:
        user => "signet",
        type => "ssh-rsa",
        key => "",
        name => "kmeijs@signet.nl_3",
        options => [],
    }      
    add_ssh_key { signet_ssh_4:
        user => "signet",
        type => "ssh-rsa",
        key => "",
        name => "_4",
        options => [],
    }      
    add_ssh_key { signet_ssh_5:
        user => "signet",
        type => "ssh-rsa",
        key => "",
        name => "cardno:000500000873_5",
        options => [],
    }      
    add_ssh_key { signet_ssh_6:
        user => "signet",
        type => "ssh-rsa",
        key => "",
        name => "bart@pc-bart_6",
        options => [],
    }      
    add_ssh_key { signet_ssh_7:
        user => "signet",
        type => "ssh-rsa",
        key => "",
        name => "adrie@d-Adrie_7",
        options => [],
    }      
    add_ssh_key { signet_ssh_8:
        user => "signet",
        type => "ssh-rsa",
        key => "",
        name => "cardno:000500000C32_signet_ssh_8",
        options => [],
    } 
##### END signet #####

##### BEGIN tripleit #####

    add_user { tripleit:
       email    => "support@triple-it.nl",
       company => "Triple IT BV",
       uid      => "5028",
       groups   => ['ring-users'],
    }
    add_ssh_key { tripleit_ssh_1:
        user => "tripleit",
        type => "ssh-rsa",
        key => "",
        name => "Triple_1",
        options => [],
    } 

##### END tripleit #####

##### BEGIN jaguarnetwork #####

    add_user { jaguarnetwork:
       email    => "noc@as30781.net",
       company => "Jaguar Networks",
       uid      => "5029",
       groups   => ['ring-users'],
    }
    add_ssh_key { jaguarnetwork_ssh_1:
        user => "jaguarnetwork",
        type => "ssh-rsa",
        key => "",
        name => "alexandre@alexandre-G5210fr_1",
        options => [],
    }      
    add_ssh_key { jaguarnetwork_ssh_2:
        user => "jaguarnetwork",
        type => "ssh-rsa",
        key => "",
        name => "matoa@general-lee_2",
        options => [],
    }      
    add_ssh_key { jaguarnetwork_ssh_3:
        user => "jaguarnetwork",
        type => "ssh-rsa",
        key => "",
        name => "nico@olivia.as30781.net.home_3",
        options => [],
    }      
    add_ssh_key { jaguarnetwork_ssh_4:
        user => "jaguarnetwork",
        type => "ssh-rsa",
        key => "",
        name => "nico@radium-as30781-net.local_4",
        options => [],
    }      
    add_ssh_key { jaguarnetwork_ssh_5:
        user => "jaguarnetwork",
        type => "ssh-rsa",
        key => "",
        name => "jerome@macbookpro.local_5",
        options => [],
    } 

##### END jaguarnetwork #####

##### BEGIN tuxis #####

    add_user { tuxis:
       email    => "mark@tuxis.nl",
       company => "Tuxis Internet Engineering",
       uid      => "5030",
       groups   => ['ring-users'],
    }
    add_ssh_key { tuxis_ssh_1:
        user    => "tuxis",
        type    => "ssh-rsa",
        key     => "",
        name    => "mark@tuxis",
        options => [],
    }


##### END tuxis #####

##### BEGIN tenet #####

    add_user { tenet:
       email    => "noc@tenet.ac.za",
       company => "TENET",
       uid      => "5031",
       groups   => ['ring-users'],
    }
    add_ssh_key { tenet_ssh_1:
        user    => "tenet",
        type    => "ssh-rsa",
        key     => "",
        name    => "noc@tenet.ac.za",
        options => [],
    }

##### END tenet #####

#### BEGIN bigwells #####

    add_user { bigwells:
        email   => "neteng@bigwells.net",
        company => "Bigwells",
        uid     => "5032",
        groups  => ['ring-users'],
    }
    add_ssh_key { bigwells_1:
        user    => "bigwells",
        type    => "ssh-rsa",
        key     => "",
        name    => "grizz@sys0.dipd.com",
        options => [],
    }

#### END bigwells #####

#### BEGIN atrato #####

    add_user { atrato:
        email   => "nmc@atratoip.net",
        company => "Atrato IP Networks",
        uid     => "5033",
        groups  => ['ring-users'],
    }

#### END atrato #####

#### BEGIN cyso #####

    add_user { cyso:
        email   => "noc@cyso.net",
        company => "Cyso",
        uid     => "5034",
        groups  => ['ring-users'],
    }
    add_ssh_key { cyso_ssh_1:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "stanley.westerveld+20110131@cyso.nl_1",
    }      
    add_ssh_key { cyso_ssh_2:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "bert.driehuis+20110117@cyso.nl_2",
    }      
    add_ssh_key { cyso_ssh_3:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "randy.tenhave+20110131@cyso.nl_3",
    }      
    add_ssh_key { cyso_ssh_4:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "ron.ursem+20110131@cyso.nl_4",
    }      
    add_ssh_key { cyso_ssh_5:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "janjorre.laurens+20110201@cyso.nl_5",
    }      
    add_ssh_key { cyso_ssh_6:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "tjebbe.dewinter+20110202@cyso.nl_6",
    }      
    add_ssh_key { cyso_ssh_7:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "nick.douma+20110207@cyso.nl_7",
    }      
    add_ssh_key { cyso_ssh_8:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "peter.geissler+20110211@cyso.nl_8",
    }      
    add_ssh_key { cyso_ssh_9:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "mark.kraakman+20110211@cyso.nl_9",
    }      
    add_ssh_key { cyso_ssh_10:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "mark.boekelaar+20110211@cyso.nl_10",
    }      
    add_ssh_key { cyso_ssh_11:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "tristan.reemer+20110211@cyso.nl_11",
    }      
    add_ssh_key { cyso_ssh_12:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "jeffry.visser+20110211@cyso.nl_12",
    }      
    add_ssh_key { cyso_ssh_13:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "paul.bankert+20110211@cyso.nl_13",
    }      
    add_ssh_key { cyso_ssh_14:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "reinier.sierag+20110211@cyso.nl_14",
    }      
    add_ssh_key { cyso_ssh_15:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "jeroen.bakker+20110211@cyso.nl_15",
    }      
    add_ssh_key { cyso_ssh_16:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "daphne.kuipers+20110215@cyso.nl_16",
    }      
    add_ssh_key { cyso_ssh_17:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "sven.visser+20110328@cyso.nl_17",
    }      
    add_ssh_key { cyso_ssh_18:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "barry.schoorl+20110401@cyso.nl_18",
    }      
    add_ssh_key { cyso_ssh_19:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "frank.muller+20110516@cyso.nl_19",
    }      
    add_ssh_key { cyso_ssh_20:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "franklin.hink+20110526@cyso.nl_20",
    }      
    add_ssh_key { cyso_ssh_21:
        user => "cyso",
        options => "from=\"85.158.200.33,93.94.224.189,62.58.71.5,82.94.220.246,200.124.139.252,172.16.200.1\"",
        type => "ssh-rsa",
        key => "",
        name => "erwin.bleeker+20110601@cyso.nl_21",
    }      
    add_ssh_key { cyso_ssh_22:
        user => "cyso",
        options => "from=\"85.158.200.*,85.158.201.*,85.158.202.*,85.158.203.*,85.158.204.*,85.158.205.*,85.158.206.*,85.158.207.*,93.94.224.*,93.94.225.*,93.94.226.*,93.94.227.*,93.94.228.*,93.94.229.*,93.94.230.*,93.94.231.*,109.235.72.*,109.235.73.*,109.235.74.*,109.235.75.*,109.235.76.*,109.235.77.*,109.235.78.*,109.235.79.*,217.170.2.*,217.170.3.*,172.16.*,10.*\"",
        type => "ssh-rsa",
        key => "",
        name => "backup+20110705@cyso.nl_22",
    }   
#### END cyso #####

#### BEGIN tdc #####

    add_user { tdc:
        email   => "noc@tdc.fi",
        company => "TDC",
        uid     => "5035",
        groups  => ['ring-users'],
    }
    add_ssh_key { tdc_ssh_1:
        user => "tdc",
        type => "ssh-rsa",
        key => "",
        name => "ytti@ytti.fi_1",
        options => [],
    }      
    add_ssh_key { tdc_ssh_2:
        user => "tdc",
        type => "ssh-dss",
        key => "",
        name => "aki.tuomi@tdc.fi_4_2",
        options => [],
    }      
    add_ssh_key { tdc_ssh_3:
        user => "tdc",
        type => "ssh-rsa",
        key => "",
        name => "nihb@tdc.dk_2_3",
        options => [],
    }      
    add_ssh_key { tdc_ssh_4:
        user => "tdc",
        type => "ssh-dss",
        key => "",
        name => "sonny@unix.dk_1_4",
        options => [],
    }      
    add_ssh_key { tdc_ssh_5:
        user => "tdc",
        type => "ssh-dss",
        key => "",
        name => "anders.kalrsson@tdc.se_3_5",
        options => [],
    }      
    add_ssh_key { tdc_ssh_6:
        user => "tdc",
        type => "ssh-dss",
        key => "",
        name => "lyngbol@freesbee.wheel.dk_6",
        options => [],
    }      
    add_ssh_key { tdc_ssh_7:
        user => "tdc",
        type => "ssh-dss",
        key => "",
        name => "Michael_7",
        options => [],
    }      
    add_ssh_key { tdc_ssh_8:
        user => "tdc",
        type => "ssh-dss",
        key => "",
        name => "mikko1_8",
        options => [],
    }      
    add_ssh_key { tdc_ssh_9:
        user => "tdc",
        type => "ssh-dss",
        key => "",
        name => "nohaneik@umma_9",
        options => [],
    }      
    add_ssh_key { tdc_ssh_11:
        user => "tdc",
        type => "ssh-rsa",
        key => "",
        name => "FC1420101222_11",
        options => [],
    }  

#### END tdc #####

#### BEGIN netability #####

    add_user { netability:
        email   => "nick@netability.ie",
        company => "Netability",
        uid     => "5036",
        groups  => ['ring-users'],
    }
    add_ssh_key { netability_0:
        user    => "netability",
        type    => "ssh-dss",
        key     => "",
        name    => "nick@pancake.netability.ie",
        options => [],
    }

#### END netablity #####


#### BEGIN unilogicnetworks #####

    add_user { unilogicnetworks:
        email   => "noc@unilogicnetworks.net",
        company => "Unilogic Networks",
        uid     => "5037",
        groups  => ['ring-users'],
    }
    add_ssh_key { unilogicnetworks_0:
        user    => "unilogicnetworks",
        type    => "ssh-dss",
        key     => "",
        name    => "cliff@dropkick",
        options => [],
    }

##### END unilogicnetworks #####

#### BEGIN maverick #####

    add_user { maverick:
        email   => "noc@maverick.com.pl",
        company => "Maverick",
        uid     => "5038",
        groups  => ['ring-users'],
    }
    add_ssh_key { maverick_ssh_1:
        user => "maverick",
        type => "ssh-dss",
        key => "",
        name => "maverick@maverick01.ring.nlnog.net_1",
        options => [],
    }      
    add_ssh_key { maverick_ssh_2:
        user => "maverick",
        type => "ssh-dss",
        key => "",
        name => "_2",
        options => [],
    } 
##### END maverick #####

#### BEGIN acsystemy #####

    add_user { acsystemy:
        email => "noc@acsystemy.pl",
        company => "AC Systemy",
        uid => "5039",
        groups => ['ring-users'],
    }
    add_ssh_key { acsystemy_ssh_1:
        user => "acsystemy",
        type => "ssh-rsa",
        key => "",
        name => "alchemyx@cerber_1",
        options => [],
    }      
    add_ssh_key { acsystemy_ssh_2:
        user => "acsystemy",
        type => "ssh-rsa",
        key => "",
        name => "barkoz@sauron_2",
        options => [],
    } 

#### END acsystemy #####

#### BEGIN netsign #####

    add_user { netsign:
        email => "noc@netsign.eu",
        company => "Netsign",
        uid => "5040",
        groups => ['ring-users'],
    }

    # removed this key per request of frapzzt
    add_ssh_key { netsign_ssh_1:
        user => "netsign",
        type => "ssh-rsa",
        key => "",
        name => "kre@netsign.eu_1",
        options => [],
        ensure => absent,
    }
    add_ssh_key { netsign_ssh_2:
        user => "netsign",
        type => "ssh-rsa",
        key => "",
        name => "kre@netsign.eu_2",
        options => [],
    }
##### END netsign #####

#### BEGIN rrbone #####

    add_user { rrbone:
        email => "noc@rrbone.net",
        company => "rrbone",
        uid => "5041",
        groups => ['ring-users'],
    }
    add_ssh_key { rrbone_ssh_1:
        user => "rrbone",
        type => "ssh-rsa",
        key => "",
        name => "libelle_1",
        options => [],
    }      
    add_ssh_key { rrbone_ssh_2:
        user => "rrbone",
        type => "ssh-rsa",
        key => "",
        name => "courante_2",
        options => [],
    }      
    add_ssh_key { rrbone_ssh_3:
        user => "rrbone",
        type => "ssh-rsa",
        key => "",
        name => "cardno:000500000CD6_3",
        options => [],
    }      
    add_ssh_key { rrbone_ssh_4:
        user => "rrbone",
        type => "ssh-rsa",
        key => "",
        name => "desirez-13092011_4",
        options => [],
    }      
    add_ssh_key { rrbone_ssh_5:
        user => "rrbone",
        type => "ssh-rsa",
        key => "",
        name => "cardno:000500000CD5_5",
        options => [],
    }      
    add_ssh_key { rrbone_ssh_6:
        user => "rrbone",
        type => "ssh-dss",
        key => "",
        name => "immo@benabuFaUl_6",
        options => [],
    }      
    add_ssh_key { rrbone_ssh_7:
        user => "rrbone",
        type => "ssh-rsa",
        key => "",
        name => "immo@T60_7",
        options => [],
    }      
    add_ssh_key { rrbone_ssh_8:
        user => "rrbone",
        type => "ssh-rsa",
        key => "",
        name => "lucas@luwo_8",
        options => [],
    }      

##### END rrbone #####

##### BEGIN hosteam #####

    add_user { hosteam:
        email => "noc@hosteam.pl",
        company => "HosTeam",
        uid => "5042",
        groups => ['ring-users'],
    }
    add_ssh_key { hosteam_ssh_1:
        user => "hosteam",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABJQAAAIEAs1ACqSoo3R4LvDgxGUuRP3EGGgmKoLwGnRdOVBLBEjI649eodKbC42hlbJLITI4inHM6KtLsZTmcwX3nSOr9Mlc+rm0SG0iRHhvGpGJ/TUswGR+e4ZiEcrlXoQEXX3Z3VaFESlR8Qoy1WA7N1//bG0piwa7z0WSOtfPwZmWXKGE=",
        name => "rsa-key-20080513_1",
        options => [],
    }

##### END hosteam #####

##### BEGIN msp #####

    add_user { msp:
        email => "noc@mediasp.com",
        company => "Media Service Provider",
        uid => "5043",
        groups => ['ring-users'],
    }
    add_ssh_key { msp_ssh_1:
        user => "msp",
        type => "ssh-dss",
        key => "",
        name => "charlie@playloudermsp.com",
        options => [],
    }

##### END msp #####

##### BEGIN inotel #####

    add_user { inotel:
        email => "noc@inotel.pl",
        company => "Inotel",
        uid => "5044",
        groups => ['ring-users'],
    }
    add_ssh_key { inotel_ssh_1:
        user => "inotel",
        type => "ssh-rsa",
        key => "",
        name => "sniegul@Debian_1",
        options => [],
    }      
    add_ssh_key { inotel_ssh_2:
        user => "inotel",
        type => "ssh-rsa",
        key => "",
        name => "sniegul@stuff.inotel.pl_2",
        options => [],
    }      
    add_ssh_key { inotel_ssh_3:
        user => "inotel",
        type => "ssh-rsa",
        key => "",
        name => "gigi@tauri_3",
        options => [],
    }      
    add_ssh_key { inotel_ssh_4:
        user => "inotel",
        type => "ssh-rsa",
        key => "",
        name => "szymon@sjazy-inotel_4",
        options => [],
    }      
    add_ssh_key { inotel_ssh_5:
        user => "inotel",
        type => "ssh-rsa",
        key => "",
        name => "rozie@pluto_5",
        options => [],
    }  
##### END inotel #####

##### BEGIN fremaks ##### 

    add_user { fremaks:
        email => "noc@fremaks.de",
        company => "Fremaks GmbH",
        uid => "5045",
        groups => ['ring-users'],
    }
    add_ssh_key { fremaks_ssh_1:
        user => "fremaks",
        type => "ssh-rsa",
        key => "",
        name => "morpheus@mbp",
        options => [],
    }      

##### BEGIN blix #####

    add_user { blix:
        email => "support@blixsolutions.eu",
        company => "Blix Solutions",
        uid => "5046",
        groups => ['ring-users'],
    }
    add_ssh_key { blix_01:
        user => "blix",
        type => "ssh-rsa",
        key => "",
        name => "root@jumpstation.blixbone.net",
        options => [],
    }

##### END blix #####

##### BEGIN portlane #####

    add_user { portlane:
        email => "noc@portlane.com",
        company => "Portlane",
        uid => "5047",
        groups => ['ring-users'],
    }
    add_ssh_key { portlane_01:
        user => "portlane",
        type => "ssh-rsa",
        key => "",
        name => "tobias@irc",
        options => [],
    }

##### END portlane #####

##### BEGIN solido #####

    add_user { solido:
        email => "noc@solido.net",
        company => "Solido",
        uid => "5048",
        groups => ['ring-users'],
    }
    add_ssh_key { solido_ssh_1:
        user => "solido",
        type => "ssh-dss",
        key => "",
        name => "hlk@solido.net_1",
        options => [],
    }      
    add_ssh_key { solido_ssh_2:
        user => "solido",
        type => "ssh-rsa",
        key => "",
        name => "jes@jes-desktop_2",
        options => [],
    }      
    add_ssh_key { solido_ssh_3:
        user => "solido",
        type => "ssh-rsa",
        key => "AAAAB3NzaC1yc2EAAAABJQAAAIEAiTxKsttTlWM3Tts99pmIm30BtBWf9bDHvY7DzH2WQAQR64CMhmNdQBVda3GHCOzc36jKt7+lmcgGlUJ/1omlKixm+URXiuUzMdvdhgvkmWd/EQ/1d5izHfx9xzDK4Rsqho0dqroC0jNBw1wDjJ9k6O5CqgwRSnYC1V/yxtpfAYs=",
        name => "jkr-solido_3",
        options => [],
    }      
    add_ssh_key { solido_ssh_4:
        user => "solido",
        type => "ssh-dss",
        key => "",
        name => "tko@solido.net_4",
        options => [],
    }

##### END solido #####

##### BEGIN digmia #####

    add_user { digmia:
        email => "ring@digmia.com",
        company => "Digmia",
        uid => "5049",
        groups => ['ring-users'],
    }
    add_ssh_key { digmia_01:
        user => "digmia",
        type => "ssh-rsa",
        key => "", 
        name => "digmia@services",
        options => [],
    }
    add_ssh_key { digmia_ssh_2:
        user => "digmia",
        type => "ssh-rsa",
        key => "",
        name => "digmia@100929_1",
        options => [],
    }
##### END digmia #####

##### BEGIN rootlu #####

    add_user { rootlu:
        email => "noc@as5577.net",
        company => "ROOT S.A.",
        uid => "5050",
        groups => ['ring-users'],
    }
    add_ssh_key { rootlu_ssh_1:
        user => "rootlu",
        type => "ssh-rsa",
        key => "",
        name => "mullebutz@mullebutz-desktop-root_1",
        options => [],
    }
    add_ssh_key { rootlu_ssh_2:
        user => "rootlu",
        type => "ssh-rsa",
        key => "",
        name => "andy@bierlair.com",
        options => [],
    }

##### END rootlu #####

##### BEGIN globalaxs #####

    add_user { globalaxs:
        email => "nmc@m247.com",
        company => "GlobalAXS Communications Ltd",
        uid => "5051",
        groups => ['ring-users'],
    }
    add_ssh_key { globalaxs_ssh_1:
        user => "globalaxs",
        type => "ssh-rsa",
        key => "", 
        name => "shafez@jumper.uk.gbxs.net",
        options => [],
    }
    add_ssh_key { globalaxs_ssh_2:
        user => "globalaxs",
        type => "ssh-rsa",
        key => "",
        name => "shafez@jumper",
        options => [],
    }

##### END globalaxs #####

}
