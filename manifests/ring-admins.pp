class ring_admins {

    add_user { job:
        email    => "job@instituut.net",
        uid      => 6001,
        groups   => ['ring-admins', 'ring-users']
    }

    add_ssh_key { job_nlnog:
    user    => "job",
    key     => "",
        name    => "job@instituut.net-nlnogring",
        type    => "ssh-rsa",
        options => "",
    }

    add_user { martin:
        email    => "martin@rodecker.nl",
        uid      => 6002,
        groups   => ['ring-admins', 'ring-users']
    }
    add_ssh_key { martin:
        user    => "martin",
        key     => "",
        name    => "martin@rodecker.nl",
        type    => "ssh-rsa",
        options => "",
    }
    
    add_user { edwin:
        email    => "edwin@nettech.cx",
        uid      => 6003,
        groups   => ['ring-admins', 'ring-users']
    }
    add_ssh_key { edwin:
        user    => "edwin",
        key     => "",
        name    => "madeddie@kyra",
        type    => "ssh-dss",
        options => "",
    }
    
    add_user { peter:
        email    => "peter@7bits.nl",
        uid      => 6004,
        groups   => ['ring-admins', 'ring-users']
    }
    add_ssh_key { peter:
        user    => "peter",
        key     => "",
        name    => "peter@onion.home.dataloss.nl",
        type    => "ssh-dss",
        options => "",
    }

}
